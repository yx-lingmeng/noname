...
const translates = {
	...
	dcbianzhuang: "变装",
	dcbianzhuang_info:
		"①出牌阶段限一次，你可以从系统随机选择的两个技能中获得一个，并视为使用一张【杀】（无距离次数限制），然后失去以此法获得的技能。②当你使用装备牌后，你清空此技能的发动次数记录。③当你发动〖变装①〗后，若你发动〖变装①〗的次数大于2，则你换肤为武·朱铁雄，并将系统选择的技能数改为三个。",
	dc_caocao: "经典曹操",
	dc_caocao_prefix: "经典",
	dcjianxiong: "奸雄",
	...
	dcbenxi: "奔袭",
	dcbenxi_info:
		"转换技，锁定技。当你失去手牌后，阳：系统随机检索出一句转换为拼音后包含“wu,yi”的技能台词，然后你念出此台词。阴：你获得上次所念出的台词对应的技能直到你的下个回合开始；若你已拥有该技能，则改为对其他角色各造成1点伤害。",
	quyuan: "屈原",
	dcqiusuo: "求索",
	dcqiusuo_info: "当你造成或受到伤害后，你可以从牌堆或弃牌堆中获得一张【铁索连环】。",
	dclisao: "离骚",
	dclisao_info:
		"出牌阶段限一次，你可以选择至多两名角色，这些角色须同时回答《离骚》的句段填空（抢答题，一名角色回答正确后结束答题）。回答正确的角色展示所有手牌，其余角色本回合不能响应你使用的牌且受到的伤害翻倍。",
	xin_sunquan: "会玩孙权",
	xin_sunquan_ab: "孙权",
	dchuiwan: "会玩",
	...
	olhuyi: "虎翼",
	olhuyi_info:
		"①游戏开始时，你从随机三个五虎将技能中选择一个获得。②当你使用或打出一张基本牌后，若你因此技能获得的技能数小于5，你随机获得一个技能描述中包含此牌名的五虎将技能。③回合结束时，你可以失去一个以此法获得的技能，然后你观看牌堆顶三张牌并获得其中一张。",
	liuxiecaojie: "刘协曹节",
	dcjuanlv: "眷侣",
	dcjuanlv_info: "当你使用牌指定异性角色为目标后，你可以令其选择一项：①弃置一张牌；②令你摸一张牌。",
	dcqixin: "齐心",
	dcqixin_info:
		"转换技。①出牌阶段，你可以将性别变更为：阳，曹节--女；阴，刘协--男。②当你即将死亡时，你取消之并将性别变更为〖齐心①〗的转换状态，将体力调整至此状态的体力，然后你本局游戏不能发动〖齐心〗。",
	dcqixin_faq: "关于齐心",
	...
	oldingxi: "定西",
	oldingxi_info:
		"当你不因〖定西〗使用伤害牌进入弃牌堆后，你可以对上家使用其中一张伤害牌，然后将使用的牌置于武将牌上。结束阶段，你摸X张牌（X为你的“定西”牌数）。",
	olnengchen: "能臣",
	olnengchen_info: "锁定技，当你受到牌造成的伤害后，若你拥有与此牌牌名相同的“定西”牌，则你随机获得其中一张。",
	olhuojie: "祸结",
	olhuojie_info:
		"锁定技，出牌阶段开始时，若X大于游戏人数，则你进行X次【闪电】判定直到以此法受到伤害（X为你的“定西”牌数）；然后若你因此受到了伤害，则你获得所有“定西”牌。",
	jiangziya: "姜子牙",
	xingzhou: "兴周",
	xingzhou_info:
		"每回合限一次，手牌数最少的角色受到伤害后，你可以弃置两张手牌，视为对伤害来源使用一张【杀】；若其因此死亡，你令〖列神〗视为未发动过。",
	lieshen: "列神",
	...
	dcbeijin: "北进",
	dcbeijin_info:
		"出牌阶段，你可以摸一张牌（此牌使用时无次数限制）。若如此做，本回合你下次使用牌或发动此技能时，若你的手牌中有以此法得到的牌，则你失去1点体力。",
	ol_jsrg_lvbu: "吕布",
	...
	dcbaoshi: "暴食",
	dcbaoshi_info:
		"摸牌阶段结束时，你可以亮出牌堆顶的两张牌。若亮出牌的牌名字数之和小于等于10（【桃】或【酒】不计入牌名字数统计），你选择一项: 1、获得所有亮出的牌; 2、再亮出一张。",
	dcxinggong: "兴功",
	...
	get olsuichong_info() {
		...
		return (
			"①游戏开始时，你可以发起一次“拼手气”红包；②每局游戏限三次，准备阶段，你可以" +
			(cost ? "消耗10金币并" : "") +
			"发起一次“拼手气”红包。运气最好的角色从三个生肖兽技能中选择一个令你获得（覆盖之前你以此法获得的生肖兽技能）。"
		);
	},
	olshouhun: "兽魂",
	olshouhun_info:
		"锁定技。①摸牌阶段你额外摸[0]张牌，你的手牌上限+[1]，你的体力上限+[2]。②当你受到伤害时，你令此技能数值最低的一项数值+1（每项数值至多为4）。",
	olzishu: "子鼠",
	...
	olyinhu: "寅虎",
	olyinhu_info:
		"出牌阶段，你可以弃置一张牌（本阶段以此法弃置的牌的类型须各不相同），然后对一名其他角色造成1点伤害。若有角色因此进入濒死状态，则此技能于本回合失效。",
	olmaotu: "卯兔",
	...
	olpimi: "披靡",
	olpimi_info:
		"你使用牌指定其他角色为唯一目标后，或成为其他角色使用牌的唯一目标后，你可以弃置使用者一张牌，令此牌伤害或回复值+1。然后若使用者的手牌最多或最少，你摸一张牌且此技能本回合失效。",
	ol_xiahouen: "OL夏侯恩",
	...
	olfulu: "负掳",
	olfulu_info:
		"①你使用【杀】结算后，你可以交给其中一名目标角色一张手牌，然后获得其至多两张手牌。②体力小于你的角色使用【杀】结算后，若此【杀】的目标包括你，其可以交给你一张手牌，然后获得你至多两张手牌。",
	tianji: "田忌",
	dcweiji: "围计",
	dcweiji_info:
		"当你使用牌指定其他角色为目标后，你可以从1-3中选择一个数字，然后令一名其他目标角色从1-3中选择一个数字，若你与其选择的数字不同，则你摸你选择数字张牌。",
	dcsaima: "赛马",
	...
	dclieti: "烈悌",
	dclieti_info:
		"锁定技，游戏开始时，你获得两组初始手牌并增加武将名称标记；当你获得手牌时，根据当前武将为手牌增加武将名标记。你只能使用或打出当前武将标记的手牌。非当前武将标记的手牌不计手牌上限。",
	dcshigong: "世公",
	dcshigong_info:
		"锁定技，你每回合使用的第一张手牌不受〖烈悌〗限制，然后你切换为对应标记的武将。若你因此切换为袁绍，你视为使用一张【万箭齐发】；若你因此切换为袁术，你摸两张牌。",
	dcluankui: "乱魁",
	dcluankui_info:
		"你每回合第二次造成伤害后，可以弃置一张「袁绍」牌令自己本回合下次造成的伤害翻倍；你每回合第二次获得牌后，可以弃置一张「袁术」牌令自己本回合下次摸牌翻倍。",
	...
	oldigong: "地公",
	oldigong_info:
		"锁定技，你使用没有「地公」的牌时，若为伤害牌，此牌伤害+1，若不为伤害牌，结算后当前回合角色判定，若为红色，你摸一张牌。你获得牌时，标记为「地公」，本轮结束时移去此标记。",
	olrengong: "人公",
	...
	strongduanti: "锻体",
	strongduanti_info:
		"锁定技，①你从牌堆摸牌后，受到1点无来源伤害。②当你发动〖称象〗时，若你的体力值为全场最低，必定亮出一张【桃】或【酒】；若你的体力值为全场最高，必定亮出一张伤害牌或武器牌。",
	strongduanti_append: "<span class='text' style='font-family: yuanli'>我要验牌！</span>",
	stronglianwu: "练武",
	stronglianwu_info:
		"你使用【杀】指定唯一目标或成为【杀】的唯一目标后，使用者每满足一项便可弃置目标角色的一张牌：1.装备了武器牌；2.此【杀】受到了【酒】的影响。",
	ren_jiangwei: "忍姜维",
	...
	renqianyao: "潜曜",
	renqianyao_info:
		"限定技，回合开始时，你可以摸X张牌，视为使用一张【杀】并依次执行前X项：1.此【杀】额外指定一个目标；2.此【杀】伤害+1；3.此【杀】不可被响应。（X为游戏轮数）",
	renhuoluan: "惑乱",
	...
	renguxing: "孤星",
	renguxing_info:
		"限定技，回合开始时，你可以依次执行前X项，从牌堆或弃牌堆中：1.获得〖惑乱〗记录的牌；2.获得与〖惑乱〗记录牌点数花色均相同的所有牌；3.获得与〖惑乱〗记录牌牌名相同的牌。（X为游戏轮数，相同牌名的牌至多获得五张）",
	wild_liru: "狂李儒",
	wild_liru_prefix: "狂",
	olhuaquan: "花拳",
	olhuaquan_info:
		"锁定技，你使用牌指定其他角色后，为此牌秘密选择一个效果：1.重拳，造成的伤害+1；2.轻拳，使用后你摸一张牌。然后其他目标角色依次猜测你选择的效果。",
	olhuaquan_heavy: "重拳",
	...
	olsanou: "三殴",
	olsanou_info:
		"锁定技，其他角色受到你的伤害后或猜错〖花拳〗的效果后，你摸一张牌并令其获得一个「击倒」标记。一名角色获得至少3个「击倒」标记后移除此标记并进入“击倒”状态。“击倒”状态的角色始终跳过出牌阶段。角色于“击倒”状态时，有10张牌离开牌堆或进入弃牌堆后，其脱离“击倒”状态。",
	bigsb_dengai: "OL乐邓艾",
	...
	olqiqi: "期期",
	olqiqi_info:
		"每轮限一次，你使用牌指定目标时，若牌名字数不小于你的体力值，你可以摸两张牌并令此牌额外结算一次，然后进行判定：若结果为♥，你减少1点体力上限。",
	you_zhugeliang: "有诸葛亮",
	...
	quanjia: "劝架",
	quanjia_info:
		"其他角色于其回合内对另一名角色造成伤害后，你可以随机观看伤害来源两张手牌。若其中有【杀】，你对其使用之；若没有【杀】，则下次发动〖劝架〗时观看牌数+1。",
	...
	mbjimi: "集蜜",
	mbjimi_info:
		"锁定技，游戏开始时，所有角色将所有手牌替换为等量张【桃】或【酒】。当有【桃】或【酒】不因使用进入弃牌堆后，你获得一张字数为X的伤害牌(X为本回合【桃】、【酒】进入弃牌堆的张数)。",
	mbmaodie: "冒迭",
	mbmaodie_info:
		"锁定技，你使用牌后，若造成伤害，你本回合下一次使用的伤害牌需大于此牌字数；每回合限两次，若未造成伤害，你获得一张目标角色的初始手牌。",
	mb_muniu: "手杀木牛流马",
	mb_muniu_prefix: "手杀",
	mbshezi: "摄梓",
	...
	mbyuetan: "跃檀",
	mbyuetan_info:
		"与你距离1以内的角色成为伤害牌的目标后，你可交给其一张牌，此牌结算结束后，若其未受到伤害，你摸一张牌。你每以此法失去两张牌，你回复1点体力。",
	...
	mbzhengpeng: "征蓬",
	mbzhengpeng_info:
		"一名角色的回合结束后，你可选择一名符合条件的角色并失去X点体力（X为此前本轮本技能发动次数），其于本回合内每满足一项，你摸一张牌：受到过伤害，失去过装备牌，非当前回合角色且获得过牌。乘势：重置本技能的X，然后你获得弃牌堆中每种类型的牌各一张。",
	mb_chitu_prefix: "手杀",
	mb_chitu: "手杀赤兔",
	mbjunkui: "骏魁",
