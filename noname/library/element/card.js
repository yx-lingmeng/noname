...
class Card extends HTMLDivElement {
    ...
    if (card[2] == "sha") {
      ...
      if (nature) {
        ...
        for (let nature2 of natures) {
          name += lib.translate["nature_" + nature2] || lib.translate[nature2] || "";
          if (nature2 != "stab") {
            this.node.image.classList.add(nature2);
          }
        }
      }
      name += "杀";
  ...
  addCardtag(tag) {
    ...
    game.broadcastAll(
      function(card2, tag2) {
        ...
        if (!_status.cardtag[tag2]) {
          _status.cardtag[tag2] = [];
        }
        _status.cardtag[tag2].add(card2.cardid);
        card2.$init([card2.suit, card2.number, card2.name, card2.nature]);
      },
      card,
      tag
    );
  ...
  removeCardtag(tag) {
    ...
    game.broadcastAll(
      function(card2, tag2) {
        ...
        _status.cardtag[tag2].remove(card2.cardid);
        card2.$init([card2.suit, card2.number, card2.name, card2.nature]);
      },
      card,
      tag
  ...
  getCacheKey(similar) {
    ...
    return prefix + `${this.name}+${this.suit ? this.suit : "none"}+${this.number === void 0 ? "none" : this.number}${this.nature ? "+" + this.nature : ""}]`;
  }
  discard(bool) {
    if (!this._selfDestroyed) {
      this.fix();
