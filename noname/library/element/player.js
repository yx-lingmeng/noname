...
${body}
  ...
  $throwEmotion(target, name2, rotate) {
    ...
    var emotion = ui.create.div(
      "",
      '<div style="text-align:center"> <img src="' + lib.assetURL + "image/emotion/throw_emotion/" + name2 + '1.png"> </div>',
  ...
  trySkillAnimate(name2, popname, checkShow) {
    ...
    if (!game.online && lib.config.skill_animation_type != "off" && lib.skill[name2] && lib.skill[name2].skillAnimation) {
      ...
      this.$skill(
        lib.skill[name2].animationStr || lib.translate[name2],
        lib.skill[name2].skillAnimation,
        lib.skill[name2].animationColor,
        checkShow
      );
      return;
    }
          ...
          if (this.skin[name2] != character) {
            ...
            game.broadcastAll(
              (player, name3, character2, list2, origin2) => {
                ...
                if (goon) {
                  lib.character[character2] = get.convertedCharacter([
                    "",
                    "",
                    0,
                    [],
                    (list2.find((i2) => i2[0] == character2) || [character2, []])[1]
                  ]);
                }
  ...
  addSubPlayer(cfg) {
    ...
    game.broadcastAll(
      ...
      function(player, skill2, list2, cfg2) {
        ...
        lib.character[skill2] = [
          cfg2.sex,
          cfg2.group,
          parseFloat(cfg2.hp) + "/" + parseFloat(cfg2.maxHp) + "/" + parseFloat(cfg2.hujia),
          cfg2.skills,
          ["tempname:" + cfg2.name].concat(lib.character[cfg2.name].trashBin || [])
        ];
...
hasSkillTag：`, arg);
  ...
  $throw(card, time, init, nosource, cardsetion) {
    ...
    if (init !== false) {
      if (init !== "nobroadcast") {
        game.broadcast(
          function(player, card2, time2, init2, nosource2, cardsetion2) {
            player.$throw(card2, time2, init2, nosource2, cardsetion2);
          },
          this,
          card,
          time,
  ...
  $gain(card, log, init, cardsetion) {
    ...
    if (init !== false) {
      game.broadcast(
        function(player2, card2, init2) {
          player2.$gain(card2, false, init2, cardsetion);
        },
        this,
        card,
        init,
