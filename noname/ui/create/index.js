  ...
  arena() {
    ...
    if (lib.config.keep_awake) {
      if (window.plugins && window.plugins.insomnia) {
        ...
        __vitePreload(async () => {
          const { default: NoSleep } = await import("../../../_virtual/index4.js").then((n) => n.i);
    ...
    character: (item, type, position, noclick, node) => {
      ...
      var func = function(node2, item2) {
        ...
        if (infoitem[1]) {
          ...
          if (double2) {
            node2.node.group.innerHTML = double2.reduce(
              (previousValue, currentValue) => `${previousValue}<div data-nature="${get.groupnature(currentValue)}">${get.translation(currentValue)}</div>`,
              ""
            );
            if (double2.length > 4) {
