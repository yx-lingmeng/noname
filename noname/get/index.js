...
class Get {
  ...
  cardDescription(node, player) {
    let str = "", name2 = node.name;
    if (lib.translate[name2 + "_info"]) {
      if (lib.card[name2].type && lib.translate[lib.card[name2].type]) {
        str += "" + get.translation(lib.card[name2].type) + "牌|";
      }
      if (get.subtype(name2)) {
        str += "" + get.translation(get.subtype(name2)) + "|";
      }
      if (lib.card[name2] && lib.card[name2].addinfomenu) {
        str += "" + lib.card[name2].addinfomenu + "|";
      }
      if (get.subtype(name2) == "equip1") {
        let added = false;
    ...
    if (lib.card[name2].cardPrompt) {
      str += "" + lib.card[name2].cardPrompt(node, player) + "|";
    } else if (lib.translate[name2 + "_info"]) {
      str += "" + lib.translate[name2 + "_info"] + "|";
    }
    if (lib.translate[name2 + "_append"]) {
      str += "" + lib.translate[name2 + "_append"] + "|";
    }
  ...
  cardsetion(player, sourceEvent) {
    ...
    const { name: name2, targets, judgestr } = _status.event;
    ...
    let name1 = name2, name22 = _status.event.getParent().name, th_skill = false, evt1 = get.event(), evt2 = get.event().getParent();
    if (["lose", "loseAsync"].includes(name2) && !lib.skill[name22] && _status.event.getParent(2).name != "die" || name2 == "gain" && !info[get.event().getParent().name]) {
      name1 = _status.event.getParent(2).name;
      evt1 = _status.event.getParent(2);
      name22 = _status.event.getParent(3).name;
      evt2 = _status.event.getParent(3);
    }
    if (name1 == "compareMultiple" || name22 == "compareMultiple" || name1?.indexOf("Callback") != -1 || name22?.indexOf("Callback") != -1) {
      name1 = _status.event.getParent(4).name;
      evt1 = _status.event.getParent(4);
      name22 = _status.event.getParent(5).name;
      evt2 = _status.event.getParent(5);
    } else if (name1 == "useCard" && name22 == "chooseUseTarget" && !evt1.skill) {
      name1 = _status.event.getParent(2).name;
      evt1 = _status.event.getParent(2);
      name22 = _status.event.getParent(3).name;
      evt2 = _status.event.getParent(3);
    ...
    if (banned.includes(name22) || bannedEvent.includes(evt2.name)) {
      name22 = false;
      evt2 = false;
    }
    if (name1 && !info[name1]) {
    ...
    if (name22 && !info[name22]) {
      if ((name22 == "chooseToUse" || name22 == "chooseToRespond" || name22 == "_wuxie") && evt2.childEvents) {
        let tempEvt;
        ...
        if (tempEvt) {
          name22 = tempEvt.name;
        } else if (name22 == "chooseToUse" && info[evt2.getParent().name] && lib.skill[evt2.getParent().name] && !banned.includes(evt2.getParent().name)) {
          evt2 = evt2.getParent();
          name22 = evt2.name;
        }
      }
      if (name22.indexOf("equip_") == 0) {
        name22 = name22.slice(6);
      } else if (name22.indexOf("pre_") == 0) {
        name22 = name22.slice(4);
        if (name22.indexOf("_backup") != -1) {
          name22 = name22.slice(0, name22.indexOf("_backup"));
        }
      } else if (name22.indexOf("lose_") == 0) {
        name22 = name22.slice(5);
      } else if (name22.indexOf("_lose") != -1 && name22.length - name22.indexOf("_lose") == 5) {
        name22 = name22.slice(0, name22.length - 5);
      }
    }
    if (name1 == "useSkill") {
    ...
    if (name22 == "useSkill") {
      name22 = get.sourceSkillFor(evt2.skill);
    }
    ...
    } else if (name22 && info[name22] || evt2.skill && info[get.sourceSkillFor(evt2.skill)]) {
      if (name22 && info[name22]) {
        if (lib.card[name22] && evt2.card && evt2.card.nature && info[evt2.card.nature]) {
          eventInfo += info[evt2.card.nature];
        }
        eventInfo += info[name22];
      } else {
  ...
  inpileVCardList(filter) {
    let list = [];
    for (const name2 of lib.inpile) {
      const type = get.type(name2);
      const info = [type, "", name2];
      if (!filter || filter(info)) {
        list.push(info);
      }
      if (name2 == "sha") {
        for (const nature of lib.inpile_nature) {
          const info2 = [type, "", name2, nature];
  ...
  cardNameLength(card, player) {
    const actualCardName = lib.actualCardName, name2 = get.translation(typeof card == "string" ? card : get.name(card, player));
    return (actualCardName.has(name2) ? actualCardName.get(name2) : name2).length;
  }
  //Yingbian
  //应变
  /**
  ...
  subtypes(obj, player) {
    ...
    if (lib.card[name2].subtypes) {
      const subtypes = get.copy(lib.card[name2].subtypes);
      return subtypes;
    } else if (lib.card[name2].subtype) {
      const subtype = lib.card[name2].subtype;
      return [subtype];
    }
    return [];
  }
  ...
  characterSurname(str, defaultSurname, defaultName) {
    ...
    for (let i = 0; i < infoarr.length; i++) {
      let name2 = infoarr[i].split("|");
      if (name2[0] === "null") {
        name2[0] = defaultSurname || "";
      }
      if (name2[1] === "null") {
        name2[1] = defaultName || "某";
      }
      names.push([name2[0], name2[1]]);
    }
    return names;
  }
  /**
  ...
  bingzhu(name2) {
    if (typeof name2 != "string") {
      name2 = get.name(name2);
    }
    const list = [], info = lib.card[name2];
    if (lib.cardBingzhu[name2]) {
      list.addArray(lib.cardBingzhu[name2]);
    }
    if (info.derivation) {
   ...
   */
  character(name2, num) {
    let info = lib.character[name2];
    if (!info) {
      const pack = Object.keys(lib.characterPack).find((pack2) => name2 in lib.characterPack[pack2]);
      if (pack) {
        info = lib.characterPack[pack][name2];
      }
    }
    if (typeof num === "number") {
      if (!info) {
   ...
   * @returns { string }
   */
  characterIntro(name2) {
    if (lib.characterIntro[name2]) {
      return lib.characterIntro[name2];
    }
    var tags = get.character(name2, 4);
    if (tags) {
    ...
    while (name2.includes("_") && !lib.characterIntro[name2]) {
  ...
  rank(name2, num) {
    if (typeof name2 === "object" && name2.name) {
      name2 = name2.name;
    }
    num = num === true ? 9 : typeof num === "number" ? num : false;
    if (name2 === _status.lord) {
...
${str}`);
  ...
  prefixSpan(prefix, name2) {
    ...
    if (exists && "getSpan" in namePrefix) {
      return namePrefix.getSpan(prefix, name2);
    }
  ...
  type(obj, method, player) {
    ...
    if (method == "trick" && lib.card[name2].type == "delay") {
      return "trick";
    }
  ...
  subtype(obj, player) {
    ...
    const name2 = get.name(obj, player);
    if (!lib.card[name2]) {
      return;
    }
    let subtype = lib.card[name2].subtype;
    return subtype;
  }
  ...
  canRespond(card, player) {
    ...
    const filter = lib.respondMap[name2];
  ...
  info(item, player) {
    ...
    if (typeof item == "object") {
      var name2 = item.name;
      if (player !== false) {
        name2 = get.name(item, player);
      }
      return lib.card[name2];
    }
  }
  /**
  ...
  skillTranslation(str, player, prefix = false) {
    ...
    const name2 = lib.translate[str];
    if (!player?.getSkills("invisible", null, false).some((skill) => {
      if (!get.skillInfoTranslation(skill, player).length || lib.translate[skill] !== name2) {
        return false;
      }
    ...
    if (info?.duplicatePrefix !== void 0) {
      ...
      if (typeof prefix2 == "function") {
        return `${prefix2(player, str)}${name2}`;
      }
  ...
  skillInfoTranslation(name2, player, noHTML = true) {
    let str = (() => {
      if (player && lib.dynamicTranslate[name2]) {
        return lib.dynamicTranslate[name2](player, name2);
      }
      const str2 = lib.translate[name2 + "_info"];
      if (!str2) {
        return "";
      }
      return str2;
    ...
    if (typeof str === "string") {
      ...
      console.warn(`孩子，你${name2}的翻译传的是什么？！`);
      return "";
    }
  }
  /**
  ...
  *iterableChildNodes(node) {
  ...
  gainableSkillsName(name2, func) {
    var list = [];
    if (name2 && lib.character[name2]) {
      if (lib.character[name2].isBoss) {
        return list;
      }
      if (lib.character[name2].isHiddenBoss) {
        return list;
      }
      if (lib.character[name2].isMinskin) {
        return list;
      }
      if (lib.character[name2].isUnseen) {
        return list;
      }
      for (var skill of lib.character[name2].skills) {
        var info = lib.skill[skill];
        if (lib.filter.skillDisabled(skill)) {
          continue;
        }
        if (func && !func(info, skill, name2)) {
          continue;
        }
        list.add(skill);
      }
   ...
   */
  cardPile(name2, position, start = "top") {
   ...
   * @returns { Card | ChildNode | null }
   */
  cardPile2(name2, start) {
    return get.cardPile(name2, "cardPile", start || "top");
  }
  /**
   * 从弃牌堆获得一张牌
   * @param { function | string | object | true } name 牌的筛选条件或名字，true为任意一张牌
   ...
   * @returns { Card | ChildNode | null }
   */
  discardPile(name2, start) {
    return get.cardPile(name2, "discardPile", start || "top");
  }
  aiStrategy() {
  ...
  skillintro(name2, learn, learn2) {
    var str = "";
    var skills = get.character(name2, 3);
    var opacity;
  ...
  nodeintro(node, simple, evt, uiintro) {
    ...
    if (typeof node._customintro == "function") {
      ...
      var name2 = node.name, Vcard = node[node.cardSymbol] || false, trueCard = node;
      ...
      if (node.link?.name && lib.card[node.link.name]) {
        name2 = node.link.name;
        Vcard = node.link[node.link.cardSymbol] || false;
        trueCard = node.link;
      }
      var cardPosition = get.position(trueCard);
      if ((cardPosition === "e" || cardPosition === "j") && trueCard.viewAs && trueCard.viewAs != name2 || Vcard && (Vcard.cards.length != 1 || Vcard.cards[0].name != name2)) {
        ...
        name2 = trueCard.viewAs;
      } else {
        if (node.extraEquip) {
          name2 = node.extraEquip[1];
      ...
      if (node._banning) {
        var clickBanned = function() {
          ...
          if (banned2.includes(name2)) {
            banned2.remove(name2);
          } else {
            banned2.push(name2);
          }
        ...
        if (lib.card[name2] && lib.card[name2].type == "trick") {
        ...
        for (var i = 0; i < list.length; i++) {
          ...
          if (banned.includes(name2) == (list[i] == "zhinang_tricks")) {
        ...
        if (lib.translate[name2 + "_info"]) {
          if (!uiintro.nosub) {
            if (lib.card[name2] && lib.card[name2].derivation) {
              if (typeof lib.card[name2].derivation == "string") {
                uiintro.add('<div class="text center">来源：' + get.translation(lib.card[name2].derivation) + "</div>");
              } else if (lib.card[name2].derivationpack) {
                uiintro.add('<div class="text center">来源：' + get.translation(lib.card[name2].derivationpack + "_card_config") + "包</div>");
              }
            }
            let typeinfo = "";
            if (lib.card[name2] && lib.card[name2].unique) {
              typeinfo += "特殊" + get.translation(lib.card[name2].type) + "牌";
            } else if (lib.card[name2] && lib.card[name2].type && lib.translate[lib.card[name2].type]) {
              typeinfo += get.translation(lib.card[name2].type) + "牌";
            }
            ...
            if (lib.card[name2].unique && lib.card[name2].type == "equip") {
              if (lib.cardPile.guozhan && lib.cardPack.guozhan.includes(name2)) {
            ...
            if (lib.card[name2] && lib.card[name2].addinfomenu) {
              uiintro.add('<div class="text center">' + lib.card[name2].addinfomenu + "</div>");
            }
            if (get.subtype(name2, false) == "equip1") {
              var added = false;
              if (lib.card[name2] && lib.card[name2].distance) {
                var dist = lib.card[name2].distance;
                if (dist.attackFrom) {
                  added = true;
          ...
          if (lib.card[name2].cardPrompt) {
            var str = lib.card[name2].cardPrompt(node.link || node, get.owner(node)), placetext = uiintro.add('<div class="text" style="display:inline">' + str + "</div>");
            if (!str.startsWith('<div class="skill"')) {
              uiintro._place_text = placetext;
            }
          } else if (lib.translate[name2 + "_info"]) {
            var placetext = uiintro.add('<div class="text" style="display:inline">' + lib.translate[name2 + "_info"] + "</div>");
            if (!lib.translate[name2 + "_info"].startsWith('<div class="skill"')) {
              uiintro._place_text = placetext;
            }
          }
          ...
          if (lib.translate[name2 + "_append"]) {
            uiintro.add('<div class="text" style="display:inline">' + lib.translate[name2 + "_append"] + "</div>");
          }
  ...
  selectGroup(name2, type) {
    if (get.itemtype(name2) == "player") {
      name2 = name2.name || name2.name1;
    }
    const info = get.character(name2);
  ...
  equipResult(player, target, name2) {
    let card = name2;
    if (typeof name2 === "string") {
      card = { name: name2 };
    } else {
  ...
  damageEffect(target, player, viewer, nature) {
    ...
    var eff = get.effect(target, { name: name2 }, player, viewer);
  ...
  skinPath(name2) {
    ...
    let dbimage = null, extimage = null, modeimage = null, nameinfo = get.character(name2), gzbool = false;
    ...
    const mode = get.mode();
    if (lib.characterPack[`mode_${mode}`] && lib.characterPack[`mode_${mode}`][name2]) {
      if (mode === "guozhan") {
        if (name2.startsWith("gz_shibing")) {
          name2 = name2.slice(3, 11);
        } else {
    ...
    if (imgPrefixUrl) {
      ...
      src = `image/mode/${modeimage}/character/${name2}${ext}`;
    } else {
      src = `image/character/${gzbool ? "gz_" : ""}${name2}${ext}`;
    }
    return `${src.split("/").slice(0, -2).join("/")}/skin/${name2}/`;
  }
  /**
   * 将URL转换成相对于无名杀根目录的路径
   *
