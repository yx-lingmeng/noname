...
class Game {
  ...
  setComplexYingbianCondition(yingbianCondition, condition) {
    return game.broadcastAll(
      (yingbianCondition2, condition2) => lib.yingbian.condition.complex.set(yingbianCondition2, condition2),
      yingbianCondition,
      condition
    );
  }
  setSimpleYingbianCondition(yingbianCondition, condition) {
    return game.broadcastAll(
      (yingbianCondition2, condition2) => lib.yingbian.condition.simple.set(yingbianCondition2, condition2),
      yingbianCondition,
      condition
    );
  }
...
${error}`);
  ...
  addCharacter(name2, information) {
    const extensionName = _status.extension || information.extension, character = [
      ...
      information.skills || [],
      [
        _status.evaluatingExtension ? `db:extension-${extensionName}:${name2}.jpg` : `ext:${extensionName}/${name2}.jpg`,
        `die:ext:${extensionName}/${name2}.mp3`
      ]
    ];
    if (information.tags) {
  ...
  addCharacterPack(pack, packagename) {
    ...
    function processCharacter(content) {
      for (const name2 in content) {
        const character = get.convertedCharacter(content[name2]);
        const audiosrc = `die:ext:${extname}/${name2}.mp3`;
        let imgsrc;
        if (_status.evaluatingExtension) {
          imgsrc = `db:extension-${extname}:${name2}.jpg`;
        } else {
        ...
        if (lib.config.forbidai_user && lib.config.forbidai_user.includes(name2)) {
          lib.config.forbidai.add(name2);
        }
